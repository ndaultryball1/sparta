...
{ // When is this updated?
  ...
  for (int isp = 0; isp < nspecies; isp++) {
    ...
    for (int jsp = 0; jsp < nspecies; jsp++) {
      ...
      prefactor[isp][jsp] = cxs * pow(2.0*update->boltz*params[isp][jsp].tref/ // just happens to be set here
        params[isp][jsp].mr, params[isp][jsp].omega-0.5) /
        tgamma(2.5-params[isp][jsp].omega); // Equivalent to pi * d_ref **2 * g_ref **(2*nu)
      double beta = MAX(vscale[isp],vscale[jsp]);
      double vrm = 2.0 * cxs * beta;
      vrmgroup = MAX(vrmgroup,vrm); // Max over all pairs of species in the given pair of groups.
    }
  }
                               ...
                               Particle::OnePart *ip, Particle::OnePart *jp)
{
  ...
  double vro  = pow(vr2,1.0-params[ispecies][jspecies].omega); // Equal to v_rel ** (1-2*nu)
